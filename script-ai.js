...
// 🚀 MOVE GENERATION (FULL DETAILED LOGIC: ANTI-LOOP + FREE PAWN)
    ...
    function renderAll() {
        ...
        for(let p in pawnStacks) {
            if(pawnStacks[p] > 0) {
                ...
                txt.setAttribute("font-size", "48"); txt.setAttribute("text-anchor", "middle"); txt.setAttribute("dominant-baseline", "middle");
        ...
        Object.entries(kingPositions).forEach(([k, pos]) => {
            if(pos){
                ...
                const txt = document.createElementNS("http://www.w3.org/2000/svg","text");
                txt.setAttribute("x", x); txt.setAttribute("y", y);
                txt.setAttribute("font-size", "50"); txt.setAttribute("text-anchor", "middle"); txt.setAttribute("dominant-baseline", "middle");
    ...
    document.getElementById("resetBtn").addEventListener("click", () => { clearHighlights(); clearAIMoveLine(); location.reload(); });
    ...
    if(modal) {
        document.getElementById("aboutBtn").addEventListener("click", () => modal.style.display="flex");
